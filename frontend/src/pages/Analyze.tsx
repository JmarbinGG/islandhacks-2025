  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://ec2-50-18-124-158.us-west-1.compute.amazonaws.com/api/analyze?data=${encodeURIComponent(gradeData)}`
      );
      
      if (!response.ok) {
        throw new Error("Failed to analyze grades");
      }
      //http://localhost:8000/api/retrieve?code
      const betterresponse = await fetch(
        `http://ec2-50-18-124-158.us-west-1.compute.amazonaws.com/api/retrieve?code=${encodeURIComponent(await response.text())}`
      );
      
      if (!betterresponse.ok) {
        throw new Error("Failed to retrieve analysis");
      }
      const result = await betterresponse.text();
